    ...
    public static async Task<CheckResult> CheckAndDownloadAsync(IProgress<string>? statusProgress = null, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(repoOwner) || string.IsNullOrWhiteSpace(repoName))
            {
                ...
                return new CheckResult(UpdateResult.NotConfigured, null, null, "更新元リポジトリが未設定です。");
            }
            ...
            if (!updateManager.IsInstalled)
            {
                ...
                return new CheckResult(UpdateResult.NotInstalled, null, null, "開発環境では更新チェックできません。");
            }

            Logger.Log($"更新チェック: リポジトリ: {repoUrl}, チャンネル: {channel}");
            statusProgress?.Report("更新を確認中...");
            ...
            try
            {
                ...
                if (completedTask == timeoutTask)
                {
                    ...
                    return new CheckResult(UpdateResult.Error, null, null, "更新チェックがタイムアウトしました。");
                }
            ...
            if (updateInfo == null)
            {
                ...
                return new CheckResult(UpdateResult.NoUpdate, null, null, "最新バージョンです。");
            }

            Logger.Log("新しいバージョンを検出しました。更新をダウンロードしています...");
            statusProgress?.Report("更新をダウンロード中...");
            ...
            catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
            {
                ...
                return new CheckResult(UpdateResult.Error, null, null, "ダウンロードがタイムアウトしました。");
            }

            Logger.Log("ダウンロード完了。更新の適用準備ができました。");
            return new CheckResult(UpdateResult.Downloaded, updateInfo, updateManager, "更新をダウンロードしました。適用中...");
        }
        ...
        catch (Exception ex)
        {
            ...
            return new CheckResult(UpdateResult.Error, null, null, $"更新チェック中にエラーが発生しました。");
        }
    }
}
