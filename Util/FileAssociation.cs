...
public class FileAssociation
{
    ...
    /// <summary>
    /// Windows 11でのファイル関連付け設定の補助情報を取得する
    /// ユーザーに「既定のアプリ」での設定方法を案内するための情報
    /// </summary>
    /// <returns>設定方法の説明文</returns>
    public static string GetWindows11AssociationHelp()
    {
        return @"Windows 11では、ファイル関連付けの設定が変更されました。

設定を完了するには、以下の手順を実行してください：

1. Windows設定を開く
2. 「アプリ」→「既定のアプリ」を選択
3. 「ファイルの種類またはプロトコルをアプリに関連付ける」をクリック
4. 設定したい拡張子（例：.zip）を検索
5. 「GGEZArchiver」を選択

または、ファイルを右クリックして「プログラムから開く」→「別のプログラムを選択」から設定することもできます。";
    }
